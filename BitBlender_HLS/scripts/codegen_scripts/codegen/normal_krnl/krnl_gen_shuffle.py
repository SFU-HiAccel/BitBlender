...
"""
    ...
    def generate_shuffle_reorder(self):
        ...
        if (SMALLER_BUFF_MODE == 1):
            print(" {}: (BLOOM-110) -     USING AN OLD, ILLEGAL VERSION OF THE CODE. KENNY GET RID OF THIS PLEASE.".format(__name__))
            pass

        else:
            if (self.config.shuffle_type == Types.ShuffleType.SEPARATED_FIFO_BASED):
                print(" {}: (BLOOM-110) - Using BUFFER-FREE shuffle unit. FIFOs must be SHUFBUF_SZ".format(__name__))
            elif (self.config.shuffle_type == Types.ShuffleType.SEPARATED_BUFFER_BASED):
                print(" {}: WARNING! WARNING! WARNING: Using suboptimal buffer-based shuffle".format(__name__))
            ...
            if (self.config.shuffle_type == Types.ShuffleType.SEPARATED_FIFO_BASED):
                codeArr.append('    PEEKED_DTYPE                shufbuf[BV_NUM_PARTITIONS];' + "\n")
                codeArr.append('    #pragma HLS ARRAY_PARTITION variable=shufbuf dim=0 complete' + "\n")
            elif (self.config.shuffle_type == Types.ShuffleType.SEPARATED_BUFFER_BASED):
                codeArr.append('    PEEKED_DTYPE                shufbuf[BV_NUM_PARTITIONS][ARB_RATELIM_DISTANCE];' + "\n")
                codeArr.append('    #pragma HLS ARRAY_PARTITION variable=shufbuf dim=0 complete' + "\n")
            ...
            if (self.config.shuffle_type == Types.ShuffleType.SEPARATED_FIFO_BASED):
                ...
                codeArr.append('        shufbuf[p].bv = 0;' + "\n")
                codeArr.append('        shufbuf[p].iidx = 0;' + "\n")
                codeArr.append('        shufbuf[p].valid = 0;' + "\n")
                codeArr.append('    }' + "\n")
            elif (self.config.shuffle_type == Types.ShuffleType.SEPARATED_BUFFER_BASED):
                codeArr.append('    PEEK_EMULATOR_INIT:' + "\n")
                codeArr.append('    for (int p = 0; p < BV_NUM_PARTITIONS; ++p) {' + "\n")
                codeArr.append('        for (int i = 0; i < ARB_RATELIM_DISTANCE; ++i) {' + "\n")
            ...
            if (self.config.shuffle_type == Types.ShuffleType.SEPARATED_FIFO_BASED):
                ...
                codeArr.append('                shufbuf[PART].valid == 0  \\' + "\n")
                codeArr.append('            ) { \\' + "\n")
                codeArr.append('                read_val_p##PART = inter_shuffle_stream_p##PART.read();  \\' + "\n")
                codeArr.append('            \\' + "\n")
                codeArr.append('                shufbuf[PART].bv = read_val_p##PART.bv_val;   \\' + "\n")
                codeArr.append('                shufbuf[PART].iidx = read_val_p##PART.iidx;   \\' + "\n")
                codeArr.append('                shufbuf[PART].valid = 1;  \\' + "\n")
                codeArr.append('            }   \\' + "\n")
                codeArr.append('' + "\n")
                codeArr.append('                //#ifdef __DO_DEBUG_PRINTS__' + "\n")
                ...
                codeArr.append('            rd_buf_idx_p##PART = read_val_p##PART.iidx % ARB_RATELIM_DISTANCE; \\' + "\n")
            ...
            if (self.config.shuffle_type == Types.ShuffleType.SEPARATED_FIFO_BASED):
                ...
                codeArr.append('' + "\n")
                codeArr.append('         /*** NOTE:' + "\n")
                codeArr.append('          * The check for input_idx == next_output_idx' + "\n")
                codeArr.append('          * is necessary to make sure were looking at the right PARTITION. Any partition could have some valid data.' + "\n")
                codeArr.append('          * But only ONE partition has ***the*** valid data with next_output_idx.' + "\n")
                codeArr.append('          */ ' + "\n")
                codeArr.append('        for (int partition_idx = 0; partition_idx < BV_NUM_PARTITIONS; ++partition_idx)' + "\n")
                codeArr.append('        {' + "\n")
                codeArr.append('            if (shufbuf[partition_idx].valid    == 1 &&' + "\n")
                codeArr.append('                shufbuf[partition_idx].iidx     == next_output_idx' + "\n")
                codeArr.append('            )' + "\n")
                codeArr.append('            {' + "\n")
                ...
                codeArr.append('            BIT_DTYPE v = shufbuf[ready_partition_idx].bv;' + "\n")
                codeArr.append('' + "\n")
                codeArr.append('            write_success = reconstruct_stream.try_write(v);' + "\n")
                codeArr.append('' + "\n")
                codeArr.append('            if (write_success) {' + "\n")
                codeArr.append('                shufbuf[ready_partition_idx].valid = 0;' + "\n")
                codeArr.append('' + "\n")
                codeArr.append('                // MANOJ: Try ternary operator, it should be better.' + "\n")
                codeArr.append('                if (next_output_idx == NUM_LOADS_PER_STM) {' + "\n")
                ...
                codeArr.append('' + "\n")
                codeArr.append('                #ifdef __DO_DEBUG_PRINTS__' + "\n")
                codeArr.append('                printf("SHUFFLE ORDERING stm%d kp%d hash%d - write BV %d\\n",' + "\n")
                codeArr.append('                        stm_idx, kp_idx, shuffle_idx,' + "\n")
                ...
                codeArr.append('        /////////////////////////////' + "\n")
                codeArr.append('        bool    write_ready = 0;' + "\n")
                codeArr.append('        int     wr_buf_idx = next_output_idx % ARB_RATELIM_DISTANCE;' + "\n")
                codeArr.append('        int     ready_partition_idx = 0;' + "\n")
                codeArr.append('' + "\n")
                codeArr.append('         /*** NOTE:' + "\n")
                codeArr.append('          * The check for input_idx == next_output_idx' + "\n")
                codeArr.append('          * is necessary to make sure were looking at the right PARTITION. Any partition could have some valid data at [buf_idx].' + "\n")
                codeArr.append('          * But only ONE partition has ***the*** valid data with next_output_idx, at [buf_idx].' + "\n")
                codeArr.append('          */ ' + "\n")
                codeArr.append('        for (int partition_idx = 0; partition_idx < BV_NUM_PARTITIONS; ++partition_idx)' + "\n")
                codeArr.append('        {' + "\n")
                ...
                codeArr.append('' + "\n")
                codeArr.append('            reconstruct_stream.write(v);' + "\n")
...
void shuffle_per_hash(
    ...
    // This is a buffer for data from each partition.
    // We also introduce a NUM_STM dimension, otherwise we hang.
    // We introduce the ARB_RATELIM_DISTANCE dimension to (hopefully) improve performance.
    PEEKED_DTYPE shuffle_peek_emulator[BV_NUM_PARTITIONS][NUM_STM][ARB_RATELIM_DISTANCE];
    #pragma HLS ARRAY_PARTITION variable=shuffle_peek_emulator dim=0 complete
    ...
    #pragma HLS UNROLL
        for (int k=0; k<NUM_STM; ++k) {
        #pragma HLS UNROLL
            for (int l=0; l<ARB_RATELIM_DISTANCE; ++l) {
            #pragma HLS UNROLL
    ...
    #pragma HLS PIPELINE II=1
        ...
        #pragma HLS UNROLL
            ...
            int buf_idx = cur_input_idx % ARB_RATELIM_DISTANCE;
        ...
        for (int strm_idx = 0; strm_idx < NUM_STM; ++strm_idx)
        {
            ...
            #pragma HLS UNROLL
                ...
                int buf_idx = next_output_idx[strm_idx] % ARB_RATELIM_DISTANCE;

                /*** NOTE:
                 * The check for input_idx == next_output_idx
                 * is necessary to make sure we're looking at the right PARTITION. Any partition could have some valid data at [strm_idx][buf_idx].
                 * But only ONE partition has ***the*** valid data with next_output_idx, at [strm_idx][buf_idx].
                 */ 
            ...
            if (cur_stm_ready) {
                ...
                for (int strm_ii = 0; strm_ii < NUM_STM; ++strm_ii)
                {
                    ...
                    {
                        for (int buf_ii=0; buf_ii < ARB_RATELIM_DISTANCE; ++buf_ii)
                        {
...
"""
    ...
    def generate_separated_shuffle_wrapper(self):
        codeArr = []
        ...
        for h in range(0, self.config.num_hash):
            if (self.config.shuffle_type == Types.ShuffleType.SEPARATED_FIFO_BASED):
                codeArr.append('    tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, NUM_STM*BV_NUM_PARTITIONS, ARB_RATELIM_DISTANCE> inter_shuf{h}_stm_kp##KP_IDX;   \\'.format(h=h) + "\n")
            elif (self.config.shuffle_type == Types.ShuffleType.SEPARATED_BUFFER_BASED):
                codeArr.append('    tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, NUM_STM*BV_NUM_PARTITIONS, STM_DEPTH> inter_shuf{h}_stm_kp##KP_IDX;   \\'.format(h=h) + "\n")
            else:
                raise AssertionError("Logic error in shuffle codegenerator.")
    ...
    def generate(self):
        ...
        elif (self.config.shuffle_type == Types.ShuffleType.SEPARATED_FIFO_BASED or
              self.config.shuffle_type == Types.ShuffleType.SEPARATED_BUFFER_BASED
        ):
            codeArr.extend(self.generate_shuffle_TtoS())
            codeArr.extend(self.generate_shuffle_reorder())
            codeArr.extend(self.generate_separated_shuffle_wrapper())
